...
테스트해보니 패턴 간격 조절은 일반 패턴과 파일더 패턴 각각 
따로 조절하도록 해야할 것 같아.. 현재는 조절을 서로 같이 
사용해서 qr 인식하느데 문제가 있어.. 둥근 모서리 반경도 따로 
사용하는게 나은거 같아..
...
import streamlit as st # 웹 앱을 만들기 위한 핵심 라이브러리입니다.
import qrcode # QR 코드 생성 기능을 제공하는 라이브러리입니다.
import io # 메모리 내에서 바이트 스트림을 다루기 위한 모듈입니다. 이미지 데이터를 파일처럼 처리할 때 사용합니다.
import re # 정규표현식을 사용하여 문자열을 검증하거나 조작하는 모듈입니다.
import math # 수학 연산을 위한 모듈입니다. 별 모양과 같은 복잡한 패턴을 그릴 때 사용합니다.
import hashlib # 해시 함수를 제공하는 모듈입니다. 현재 코드에서는 사용되지 않지만, 향후 데이터 무결성 검증 등에 활용될 수 있습니다.
import base64 # 이진 데이터를 텍스트 형식(Base64)으로 인코딩/디코딩하는 모듈입니다. SVG 이미지 데이터를 웹에 표시할 때 유용합니다.
import qrcode.image.svg # QR 코드를 벡터 기반의 SVG 형식으로 생성하는 데 필요한 모듈입니다.
from datetime import datetime # 현재 시각 정보를 얻기 위한 모듈입니다. 파일명 자동 생성에 사용됩니다.
from zoneinfo import ZoneInfo # 시간대 정보를 다루는 모듈입니다. 대한민국 표준시(KST)를 설정하는 데 사용됩니다.
from messages import messages # 다국어 지원을 위해 언어별 메시지를 담고 있는 외부 딕셔너리입니다.
from PIL import Image, ImageDraw # Pillow 라이브러리의 핵심 모듈입니다. 이미지 생성, 편집, 그리고 다양한 모양을 그리는 데 사용됩니다.


# 💡 QR 코드의 오류 복원 수준 옵션을 qrcode 라이브러리의 상수 값과 매핑하는 딕셔너리입니다.
...
# `set_language` 함수에서 언어가 변경될 때 호출되어, 위젯의 선택 상태를 새로운 언어에 맞는 기본값으로 초기화합니다.
...
# `st.set_page_config`는 앱이 재실행될 때마다 호출되어, 동적으로 페이지 타이틀을 변경할 수 있게 해줍니다.
# 사용자가 언어를 변경하면 Streamlit 앱이 재실행되므로, 이 코드가 다시 실행되어 브라우저 탭 제목이 업데이트됩니다.
dynamic_page_titles = {
    "ko": "QR 코드 생성기",
...
st.set_page_config(
    page_title=dynamic_page_titles[st.session_state.get('lang', 'ko')], # 현재 세션 상태에 저장된 언어에 따라 동적으로 제목을 설정
...
def get_qr_data_object(data, box_size, border, error_correction, mask_pattern,):
    try:
        ...
        qr = qrcode.QRCode(
            version=1, # QR 코드의 버전 (1-40) - 1로 설정하면 데이터 양에 따라 자동으로 결정됩니다.
            error_correction=error_correction, # 오류 복원 수준을 설정합니다.
            box_size=box_size, # QR 코드의 각 "박스"(모듈) 크기를 픽셀 단위로 설정합니다.
            border=border, # QR 코드 주변의 테두리 두께를 모듈 단위로 설정합니다.
            mask_pattern=mask_pattern, # 마스크 패턴 번호 (0-7)를 설정합니다.
        )
...
# 사용자가 언어를 변경하면 기존 위젯의 상태를 저장하고, 새로운 언어에 맞게 변환한 뒤 복원합니다.
