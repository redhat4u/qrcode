...
with col2:
        ...
        try:
            ...
            if qr:
                ...
                preview_image_display = draw_custom_shape_image(
                    ...
                    "사각" if file_format == "SVG" else st.session_state.finder_pattern_shape_select,
                )
        except Exception as e:
            st.error(f"오류가 발생했습니다: {str(e)}")

    generate_btn = st.button("⚡ QR 코드 생성", use_container_width=True, disabled=(not current_data))
    ...
    if generate_btn:
        ...
        if not (st.session_state.file_format_select == "SVG"):
