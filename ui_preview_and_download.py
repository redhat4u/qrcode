...
def build_preview_and_download_ui():
    ...
    if st.session_state.generate_button_clicked and st.session_state.error_message:
    ...
    elif st.session_state.generate_button_clicked and st.session_state.show_generate_success:
        st.success(UI_SUCCESS_MESSAGE)
        st.markdown(
            f'<div style="text-align: center; border: 1px solid #ddd; padding: 10px; margin-bottom: 20px;">'
            f'{st.image(st.session_state.qr_image_bytes, use_column_width=True) if st.session_state.file_format_select == UI_FILE_FORMAT_PNG else st.image(st.session_state.qr_svg_bytes, use_column_width=True)}'
            f'</div>',
        ...
        st.markdown(
            ...
            f'<span style="color:darkorange; font-weight:bold;">{UI_DOWNLOAD_FILENAME} </span> '
