...
def build_preview_and_download_ui():
    ...
    if st.session_state.error_message:
        ...
        st.success("✅ QR 코드 생성 완료!!<br>반드시 파일명을 확인하고, 화면 아래의 [💾 QR 코드 다운로드] 버튼을 클릭하세요.")
    elif preview_image_display:
        st.success("현재 입력된 내용으로 생성될 QR 코드를 미리 표현해 보았습니다.<br>이 QR 코드가 맘에 드신다면, 위의 [⚡ QR 코드 생성] 버튼을 클릭하세요.")
    else:
